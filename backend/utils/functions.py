...
def get_model():
    ...
    extractive_model.load_state_dict(
      torch.load(os.path.abspath(
        "models/minilm_bal_exsum.pth"), map_location=torch.device('cpu')))
...
def predict(model, sents, doc):
    ...
    sent_id, sent_mask = torch.tensor(
        sent_id, dtype=torch.long), torch.tensor(sent_mask, dtype=torch.long)

    doc_id, doc_mask = get_tokens([doc])
    doc_id, doc_mask = doc_id * len(sents), doc_mask * len(sents)
    doc_id, doc_mask = torch.tensor(
        doc_id, dtype=torch.long), torch.tensor(doc_mask, dtype=torch.long)
...
def summarize(doc, model, min_sentence_length=14, top_k=3, batch_size=3):
    ...
    sent_pred_list = [{"sentence": doc_sentences[i], "score": scores[i]
                       [0], "index":i} for i in range(len(doc_sentences))]
